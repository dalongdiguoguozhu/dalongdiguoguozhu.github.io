<template>
  <view>
    ...
    <view class="personal_card bg-white">
      ...
      <view class="infor">
        <view class="username_position username">
          {{name}} <image src="../../static/me/VIP.png" class="ico-vip"></image>
        </view>
        <view class="intro_style"> {{intro}} </view>
      </view>
      <view class="ico-positon">
        <view
          class="cuIcon-settings ico-size"
          style=" z-index: 999;margin-left: 150rpx;"
          @click="tosettings()"
        ></view>
        <view
          class="cuIcon-message ico-size"
          style=" z-index: 999;margin-left: 50rpx;"
        ></view>
      </view>
    </view>
    <!-- 名片 end -->
    ...
    <view class="text_position bg-white">
      <view class="attention_num"> {{fanNum}} </view>
      <view class="attention font-style"> {{attention}} </view>
      <view class="attention_num"> {{fanNum}} </view>
      <view class="attention font-style"> {{fans}} </view>
    </view>
    ...
    <view class="my-order-style bg-white">
      <view class="bg-white text-box">
        <view class="text-myorder"> {{myorder}} </view>
        <view class="text-all" @click="Toall()"> {{toall}} </view>
      </view>
      ...
      >
        ...
        <swiper-item class="swiper-item bg-white colletc-flex">
          <view
            ...
            :key="index"
          >
            <button type="" class="collect_btn" @click="toCollect">
              {{ item.name }}
            </button>
            <view class="img-flex">
    ...
    <!-- 局部切换 end -->
    <!-- 底部导航栏 start -->
    <view class="bottombar"> <bottombar :tIndex="4"></bottombar> </view>
